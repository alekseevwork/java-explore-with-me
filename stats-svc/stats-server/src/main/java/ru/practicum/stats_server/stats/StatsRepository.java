...
public interface StatsRepository extends JpaRepository<EndpointsHit, Long> {
    ...
    @Query("SELECT new ru.practicum.stats_server.stats.model.ViewStats(h.app, h.uri, COUNT(DISTINCT(h.ip)) AS hits) " +
    ...
    @Query("SELECT new ru.practicum.stats_server.stats.model.ViewStats(h.app, h.uri, COUNT(h.ip) AS hits) " +
            "FROM EndpointsHit AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end " +
            "GROUP BY h.app, h.uri ORDER BY hits DESC")
