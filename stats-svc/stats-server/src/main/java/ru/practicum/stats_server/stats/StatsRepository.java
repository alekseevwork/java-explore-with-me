...
public interface StatsRepository extends JpaRepository<EndpointsHit, Long> {
    ...
    @Query("SELECT new ru.practicum.stats_server.stats.model.ViewStats(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
    ...
    @Query("SELECT new ru.practicum.stats_server.stats.model.ViewStats(h.app, h.uri, COUNT(h.ip)) " +
            "FROM EndpointsHit AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end " +
            "GROUP BY h.app, h.uri ORDER BY COUNT(DISTINCT h.ip) DESC")
